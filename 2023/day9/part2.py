...
def find_previous_value(processed):
    processed = list(reversed(processed))
    for i, line in enumerate(processed):
        if i == 0:
            processed[i].append(0)
        else:
            processed[i] = [line[0] - processed[i - 1][0]] + processed[i]

    return processed[len(processed) - 1][0]
...
def main():
    ...
    for line in read_input().splitlines():
        numbers.append(find_previous_value(apply_instructions(process_input(line))))
    print(sum(numbers))
